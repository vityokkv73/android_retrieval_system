...
public class ActivityProvider extends ContentProvider {
    ...
    private static class DatabaseHelper extends SQLiteOpenHelper {
        ...
        public void onCreate(SQLiteDatabase db) {
            ...
            db.execSQL("CREATE TABLE " + SMS_TABLE_NAME + " ("
                    + ActivityContract.SMS._ID + " INTEGER PRIMARY KEY AUTOINCREMENT,"
                    + ActivityContract.SMS.SENDER + " TEXT,"
                    + ActivityContract.SMS.RECIPIENT + " TEXT,"
                    + ActivityContract.SMS.SENDER_PHONE_NUMBER + " TEXT,"
                    + ActivityContract.SMS.RECIPIENT_PHONE_NUMBER + " TEXT,"
                    + ActivityContract.SMS.TIME + " INTEGER,"
            ...
            db.execSQL("CREATE TABLE " + CALLS_TABLE_NAME + " ("
                    ...
                    + Calls.CALLER + " TEXT,"
                    + Calls.RECIPIENT + " TEXT,"
                    + Calls.CALLER_PHONE_NUMBER + " TEXT,"
                    + Calls.RECIPIENT_PHONE_NUMBER + " TEXT,"
                    + Calls.TIME + " INTEGER"
                    + ");");
            
            //create thumbnails table
            db.execSQL("CREATE TABLE " + THUMBNAILS_IMAGE_TABLE_NAME + " ("
                    + Thumbnails._ID + " INTEGER PRIMARY KEY AUTOINCREMENT,"
                    + Thumbnails.MEDIASTORE_ID + " INTEGER,"
                    + Thumbnails.DELETED + " BOOLEAN,"
                    + Thumbnails.THUMBNAIL_SENT + " BOOLEAN,"
    ...
    public int delete(Uri uri, String where, String[] whereArgs) {
        ...
        switch (sUriMatcher.match(uri)) {
            ...
            case THUMBNAILS_ID:
                ...
                count = db.delete(THUMBNAILS_IMAGE_TABLE_NAME, Thumbnails._ID + "=" + imageId
                        + (!TextUtils.isEmpty(where) ? " AND (" + where + ")" : ""), whereArgs);
                break;
    ...
    public int update(Uri uri, ContentValues values, String where, String[] whereArgs) {
        ...
        switch (sUriMatcher.match(uri)) {
            ...
            case SMS_ID:
                String smsId = uri.getPathSegments().get(1);
                count = db.update(SMS_TABLE_NAME, values, ActivityContract.SMS._ID + "=" + smsId
                        + (!TextUtils.isEmpty(where) ? " AND (" + where + ")" : ""), whereArgs);
                break;
            ...
            case THUMBNAILS_ID:
                String imageId = uri.getPathSegments().get(1);
                count = db.update(THUMBNAILS_IMAGE_TABLE_NAME, values, Thumbnails._ID + "=" + imageId
                        + (!TextUtils.isEmpty(where) ? " AND (" + where + ")" : ""), whereArgs);
                break;
            ...
            case LOCATIONS_ID:
                String locationId = uri.getPathSegments().get(1);
                count = db.update(LOCATION_TABLE_NAME, values, Locations._ID + "=" + locationId
                        + (!TextUtils.isEmpty(where) ? " AND (" + where + ")" : ""), whereArgs);
                break;
